    ...
    protected function task() {
        if($this->method == 'GET') {
            ...
            if((mt_rand() / mt_getrandmax()) > 0.5) {
                ...
                $seedID = $this->select("SELECT * FROM (SELECT COUNT(g.gameID) as rowCount, gu.userID as userID FROM GameUser gu INNER JOIN Game g ON g.gameID=gu.gameID TIMESTAMPDIFF(MINUTE, NOW(), g.timestamp) < 60 GROUP BY gu.userID ORDER BY rowCount / MIN(60, TIMESTAMPDIFF(MINUTE, NOW(), u.lastCompileTime)) DESC limit 15) temptable order by rand() limit 1")['userID'];
